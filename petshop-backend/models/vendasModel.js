...
const Vendas = {
    ...
    getById: (id) => db.query('SELECT * FROM vendas WHERE id_venda = ?', [id]),
    ...
    update: (id, data) =>
        db.query(
            'UPDATE vendas SET data = ?, id_cliente = ?, total = ? WHERE id_venda = ?',
            [data.data, data.id_cliente, data.total, id]
        ),

    // Deletar uma venda
    delete: (id) => db.query('DELETE FROM vendas WHERE id_venda = ?', [id]),
};
